...
create_subject(studyRoot, 'BothFormats001', session, 'acq-ax', [0 1000]);
bothDir = fullfile(studyRoot, 'rawdata', 'sub-BothFormats001', ...
    'ses-research', 'dwi');
bothNii = fullfile(bothDir, ...
    'sub-BothFormats001_ses-research_acq-ax_dwi.nii');
gzip(bothNii);
assert_error(@() mh_fiber_dwi_bids_jobspec(studyRoot, 'BothFormats001', ...
    'Session', session, 'AnchorModality', 'T2w', 'RequireT1', true), ...
    'mh_fiber_dwi_bids_jobspec:AmbiguousRawDwi');
