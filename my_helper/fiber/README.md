...
- Atlas/ROI buttons from the configured visualization atlas, `NAc_ALIC (Yu 2021 and Ewert 2017)` by default.
...
All buttons are enabled by default. Clicking a button hides its object; clicking it again shows the object. Empty fiber bundles are skipped and do not get a toolbar button. Right-clicking helper-managed atlas/ROI, VTA, fiber, or lead buttons opens a lightweight display-control window for visibility, color, edge color, and alpha when those properties are available.

The helper keeps the visualization control logic close to `ea_mnifigure`: the scene is still created by `ea_elvis`, the configured visualization atlas is passed through `options.atlasset`, and Lead-DBS creates the native atlas toolbar buttons and Atlas Control window. Helper code only rebinds the saved `.fig` callbacks so they remain functional after reopening.

When writing the `.fig`, the helper stores the main scene objects and toolbar callbacks but does not serialize transient Anatomy Slices or Atlas Control windows. These windows can be recreated after reopening the scene through `mh_fiber_open_scene(figPath)` or the native Lead-DBS toolbar buttons.
