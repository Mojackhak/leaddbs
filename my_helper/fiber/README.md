...
The standalone Python atlas builder under `core/whole_brain_roi_atlas` resamples all 198 HybraPD labels to the Lead-DBS 0.5 mm reference and exports 150 non-white-matter Lead-DBS ROI masks plus 48 hidden white-matter QC masks. The anatomical atlas is named `HybraPD Whole Brain (Yu 2021)`; the accompanying dTOR endpoint census uses strict containing-voxel assignment and does not define atlas boundaries. See `core/whole_brain_roi_atlas/README.md` for validation, build, and status commands.
