...
### Phase 2CF Attempt 5 Full-Cohort Completion

Status: **complete and validated**.

Attempt 5 used the same full-cohort rerun configuration as attempt 4, with the
fixed atlas gray-matter mask cache and bounded process-worker missing-result
retry enabled by default.

Runtime:

- Started: `2026-07-04 08:31:24 -0700`.
- Ended: `2026-07-04 10:20:14 -0700`.
- MATLAB elapsed time: 6530.228846 seconds.
- Shell wrapper exit code: 0.
- Process work directory:
  `/Volumes/VAL/STNSNr/validation/vta_full_cohort_realroot_rerun_20260704_024228/process_tasks_attempt5`.

Completion summary:

- All 16 selected workbook subjects completed.
- Process task payload/result count: 142 payload MAT files and 142 result MAT
  files.
- Retry log count: 0.
- Frozen gray-matter mask count: 85.
- No selected-subject `.stnsnr_vta_coverage.lock` directories remained.
- Each selected subject produced:
  - `reports/*_vta_coverage_long.csv`.
  - `reports/*_contact_mapping_qc.csv`.
  - `reports/*_vta_coverage_summary.md`.
  - `manifest/*_vta_coverage_manifest.json`.
  - 48 primary thresholded coverage masks matching `*_desc-vta.nii`.
  - 48 category masks and 48 program-overlap masks.

Cohort validation:

- `cohort_vta_coverage_long.csv`: 1536 rows x 25 columns.
- `cohort_contact_mapping_qc.csv`: 194 rows x 20 columns.
- `cohort_vta_coverage_wide.csv`: 583 rows x 27 columns.
- `cohort_vta_coverage_by_condition.csv`: 48 rows x 8 columns.
- `cohort_vta_threshold_sensitivity.csv`: 384 rows x 8 columns.
- Cohort figures were regenerated:
  - `coverage_threshold_sensitivity.png`.
  - `coverage_boxplot_by_condition_thr0p20.png`.
  - `coverage_stacked_bar_thr0p20.png`.
- `target_component_distribution` remained present and was not regenerated.

Manifest validation:

- `vta_gm_atlas = Custom_Ewert_Zhang_Middlebrooks`.
- `vta_model_key = simbio`.
- `vta_execution_mode = process`.
- `vta_parallel_workers = 4`.

Validation artifacts:

- Refined validation report:
  `/Volumes/VAL/STNSNr/validation/vta_full_cohort_realroot_rerun_20260704_024228/attempt5_validation_report_refined.md`.
- Subject validation table:
  `/Volumes/VAL/STNSNr/validation/vta_full_cohort_realroot_rerun_20260704_024228/attempt5_subject_validation_refined.csv`.
- Runtime file:
  `/Volumes/VAL/STNSNr/validation/vta_full_cohort_realroot_rerun_20260704_024228/matlab_runtime_attempt5.txt`.

Validation notes:

- Refined validation status: PASS.
- No critical worker or MATLAB failure signatures were found:
  `task failure`, `timeout`, `missing VTA output`, `worker exited early`,
  `Failed to load gray matter mask`, `GUNZIP`, `Can't open image file`, and
  `A VTA task worker exited`.
- The main MATLAB log contains 1080 `Duplicate .nii/.nii.gz files detected`
  warnings during subject-space atlas materialization. These warnings did not
  occur in worker logs, did not prevent frozen-mask worker execution, and did
  not affect completion.
- `parallel_jobs_latest.csv` was not generated by attempt 5. This is recorded
  as a non-blocking validation warning because the process-mode worker logs,
  manifests, payload/result MAT files, and runtime file provide the execution
