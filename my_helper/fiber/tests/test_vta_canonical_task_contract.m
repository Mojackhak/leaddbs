...
function testStaticDefinitionDoesNotRequireRuntimeContext(testCase)
task = fixture_task('continuous_joint', 'continuous', ...
    fixture_source('source-1', 'voltage'));
task = rmfield(task, {'run_id', 'output_leaves', 'missing_artifacts'});

resolved = mh_vta_validate_canonical_task_definition(task);

verifyEqual(testCase, string(resolved.task_id), "task-001");
verifyEqual(testCase, numel(resolved.solve_units), 1);
end

function testRuntimeValidatorRejectsNoncanonicalLeaf(testCase)
task = fixture_task('continuous_joint', 'continuous', ...
    fixture_source('source-1', 'voltage'));
task = mh_vta_validate_canonical_task_definition(task);
task.output_leaves.native = '/tmp/noncanonical-leaf';

verifyError(testCase, @() mh_vta_validate_canonical_task_runtime(task), ...
    'mh_vta:InvalidCanonicalTask');
end

function testContactFractionsAndThresholdFilenamesAreValidated(testCase)
task = fixture_task('continuous_joint', 'continuous', ...
    fixture_source('source-1', 'voltage'));
task.sources(1).contacts(1).fraction = 0.5;
verifyError(testCase, ...
    @() mh_vta_validate_canonical_task_definition(task), ...
    'mh_vta:InvalidCanonicalTask');

task = fixture_task('continuous_joint', 'continuous', ...
    fixture_source('source-1', 'voltage'));
task.model.thresholds_v_per_m = [180.001, 180.002];
verifyError(testCase, ...
    @() mh_vta_validate_canonical_task_definition(task), ...
    'mh_vta:InvalidCanonicalTask');
end
