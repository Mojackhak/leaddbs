...
def _combined_condition(
    ...
    for source_id, role in condition_roles.items():
        ...
        normalized = {key: copy.deepcopy(value) for key, value in source.items() if key != source_subscale_field}
        rows.append(normalized)
    if not rows:
        raise MigrationError("combined condition mapping is empty")
    if any(row != rows[0] for row in rows[1:]):
        raise MigrationError("combined stimulation definitions conflict after removing source subscale data")
    return {
...
def _reference_condition(
    ...
    metadata = {key: copy.deepcopy(value) for key, value in source.items() if key != source_subscale_field}
    return {
...
def _convert_scales(
    ...
    for row in source_scales:
        ...
        source_bindings = _require_mapping(row.get("endpoint_bindings"), f"scale {scale_id} bindings")
...
def _convert_model(source: Mapping[str, Any], mapping: Mapping[str, Any]) -> dict[str, Any]:
...
def convert_profiles(
    ...
    if set(components) != {"reference_component", "addon_component"}:
        raise MigrationError("component mapping must produce reference_component and addon_component")
    conditions = {
    ...
    normative = _require_mapping(model.get("normative_fiber"), "source normative_fiber")
    source_connectome_roles = _require_mapping(
        normative.get("connectome_roles"), "source connectome roles"
    )
    target_connectomes: dict[str, Any] = {}
    for connectome_id, payload in _require_mapping(study.get("connectomes"), "source connectomes").items():
        source_role = source_connectome_roles.get(connectome_id)
        if source_role == "observed_robustness":
            roles = ["observed_robustness"]
        elif source_role == "formal":
            roles = ["primary_formal", "activation_sensitivity_enabled"]
        else:
            raise MigrationError(f"unknown source connectome role for {connectome_id}: {source_role}")
        target_connectomes[connectome_id] = {**copy.deepcopy(payload), "roles": roles}
    target_study = {
        "schema_version": "dual_frequency_v1",
        ...
        "space": copy.deepcopy(study.get("space")),
        "connectomes": target_connectomes,
        "run_output_root": _require_mapping(study.get("paths"), "source paths").get("output_root"),
    }
    target_scales, binding_report = _convert_scales(scales, subscale_bindings)
    target_model = _convert_model(model, mapping)
    target_normative = _require_mapping(target_model.get("normative_fiber"), "target normative_fiber")
    target_normative.pop("connectome_roles", None)
    ...
    selected_source_values = source_selection.get(source_selector, [])
    if not isinstance(selected_source_values, list):
        raise MigrationError("source subscale selection must be a list")
    ...
    target_workflow = {
        ...
        "selection": {
            "scales": copy.deepcopy(source_selection.get("scales", [])),
            "subscales": [selector_map[value] for value in selected_source_values],
    ...
    for name, payload in (
        ("study.yaml", target_study),
        ("scales.yaml", target_scales),
        ("model.yaml", target_model),
        ("workflow.yaml", target_workflow),
    ):
        _write_yaml_atomic(output_dir / name, payload)
    report = {
        ...
        "target_profiles": ["study.yaml", "scales.yaml", "model.yaml", "workflow.yaml"],
    }
