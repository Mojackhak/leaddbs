...
matlabExe = mh_fiber_getenv_default('STNSNR_MATLAB_EXE', executionDefaults.matlabExe);
...
envValues.STNSNR_VTA_GM_ATLAS = mh_fiber_getenv_default( ...
    'STNSNR_VTA_GM_ATLAS', mh_fiber_stnsnr_default_vta_gm_atlas());
envValues.STNSNR_VTA_MODEL_KEY = mh_fiber_getenv_default( ...
    'STNSNR_VTA_MODEL_KEY', mh_fiber_stnsnr_default_vta_model_key());
envValues.STNSNR_VTA_EXECUTION_MODE = mh_fiber_getenv_default( ...
    'STNSNR_VTA_EXECUTION_MODE', executionDefaults.executionMode);
envValues.STNSNR_VTA_PARALLEL_WORKERS = mh_fiber_getenv_default( ...
    'STNSNR_VTA_PARALLEL_WORKERS', num2str(executionDefaults.parallelWorkers));
...
envValues.STNSNR_VTA_TASK_CONDA_ENV = mh_fiber_getenv_default( ...
    'STNSNR_VTA_TASK_CONDA_ENV', executionDefaults.condaEnv);
envValues.STNSNR_VTA_PROCESS_WORK_DIR = mh_fiber_getenv_default( ...
    'STNSNR_VTA_PROCESS_WORK_DIR', executionDefaults.processWorkDir);
envValues.STNSNR_VTA_PROCESS_DRY_RUN = mh_fiber_getenv_default( ...
    'STNSNR_VTA_PROCESS_DRY_RUN', mat2str(executionDefaults.processDryRun));
envValues.STNSNR_VTA_PROCESS_POLL_SECONDS = mh_fiber_getenv_default( ...
    'STNSNR_VTA_PROCESS_POLL_SECONDS', num2str(executionDefaults.processPollSeconds));
envValues.STNSNR_VTA_PROCESS_TIMEOUT_SECONDS = mh_fiber_getenv_default( ...
    'STNSNR_VTA_PROCESS_TIMEOUT_SECONDS', num2str(executionDefaults.processTimeoutSeconds));

jobs = mh_vta_launch_process_workers(subjectIds, workerScript, logDir, ...
    'WorkerCount', workerCount, ...
    'MatlabExe', matlabExe, ...
    'DryRun', dryRun, ...
    'CondaEnv', executionDefaults.condaEnv, ...
    'Env', envValues);
