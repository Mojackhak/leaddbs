function boundary = mh_vta_assemble_onesolve_boundary(sources, activeidx, controlMode)
...
if ~ismember(mode, {'voltage', 'current'})
    error('mh_vta_assemble_onesolve_boundary:InvalidControlMode', ...
        'Control mode must be voltage or current.');
end
if ~isstruct(sources) || isempty(sources)
    error('mh_vta_assemble_onesolve_boundary:InvalidSources', ...
        'Sources must be a nonempty struct array.');
end
if ~isstruct(activeidx) || ~isfield(activeidx(1), 'con')
    error('mh_vta_assemble_onesolve_boundary:InvalidActiveIndex', ...
...
if numel(unique(sourceIds)) ~= numel(sourceIds)
    error('mh_vta_assemble_onesolve_boundary:DuplicateSourceId', ...
...
for sortedIndex = 1:numel(sourceOrder)
    ...
    for contactIndex = 1:numel(contacts)
        ...
        if ismember(contactNumber, usedContacts)
            error('mh_vta_assemble_onesolve_boundary:DuplicateContact', ...
...
if isempty(boundary.node_indices)
    error('mh_vta_assemble_onesolve_boundary:MissingElectrodeContact', ...
...
for index = 1:numel(sources)
    if ~isfield(sources(index), 'source_id')
        error('mh_vta_assemble_onesolve_boundary:MissingSourceId', ...
    ...
    if isempty(sourceIds{index})
        error('mh_vta_assemble_onesolve_boundary:InvalidSourceId', ...
...
elseif isfield(source, 'controlMode')
    ...
    error('mh_vta_assemble_onesolve_boundary:MissingControlMode', ...
        'Source %s does not define control_mode.', sourceId);
end
if ~strcmp(sourceMode, expectedMode)
    error('mh_vta_assemble_onesolve_boundary:MixedControlMode', ...
...
if isfield(source, 'unit')
    ...
    if (strcmp(expectedMode, 'voltage') && ~strcmpi(unit, 'v')) || ...
        ...
        error('mh_vta_assemble_onesolve_boundary:InvalidUnit', ...
...
if ~isfield(source, 'contacts') || ~isstruct(source.contacts) || ...
    ...
    error('mh_vta_assemble_onesolve_boundary:InvalidContacts', ...
...
for index = 1:numel(contacts)
    ...
    if ~all(isfield(contacts(index), requiredFields))
        error('mh_vta_assemble_onesolve_boundary:InvalidContact', ...
    ...
    if ~ismember(polarity, {'cathode', 'anode'})
        error('mh_vta_assemble_onesolve_boundary:InvalidPolarity', ...
    ...
    if ~isnumeric(fraction) || ~isscalar(fraction) || ...
        ...
        error('mh_vta_assemble_onesolve_boundary:InvalidContactFraction', ...
...
if ~any(matching)
    error('mh_vta_assemble_onesolve_boundary:MissingPolarity', ...
...
if contactNumber > numel(connections) || ...
    ...
    error('mh_vta_assemble_onesolve_boundary:MissingActiveNodes', ...
...
if ~isnumeric(nodes) || any(~isfinite(nodes)) || any(nodes < 1) || ...
        any(nodes ~= fix(nodes))
    error('mh_vta_assemble_onesolve_boundary:InvalidActiveNodes', ...
...
if ~(ischar(value) || (isstring(value) && isscalar(value)))
    error(['mh_vta_assemble_onesolve_boundary:', errorSuffix], ...
