...
if actions.transform_mni_efield
    ...
    stageTimer = tic;
    [context, cacheStatus] = mh_vta_resolve_transform_context( ...
        task, parser.Results.SubjectRuntime);
    mh_vta_emit_stage_timing(emit, 'task', taskId, ...
        'subject_reconstruction_context', 'executed', ...
        toc(stageTimer), cacheStatus);
    mh_vta_publish_atomic(mniEfield, @(temporaryPath) ...
        transform_to_mni(nativeEfield, context.options, ...
            context.mni_reference, ...
