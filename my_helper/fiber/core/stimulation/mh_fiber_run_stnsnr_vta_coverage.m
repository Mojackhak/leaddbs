...
if logical(opts.CohortOnly)
    ...
    if height(subjects) == 16
        validate_cohort_outputs(coverageTable, contactTable, cohortOutputDir, ...
            thresholdsVPerMm, regionSpec);
    else
...
mh_util_require_table_vars(rows, required, ...
    'mh_fiber_run_stnsnr_vta_coverage:MissingWorkbookColumn', ...
...
if ~isempty(continuousRows)
    ...
    programs(programIdx).label = mh_fiber_stnsnr_vta_program_label( ...
...
for i = 1:height(alternatingRows)
    ...
    programs(programIdx).label = mh_fiber_stnsnr_vta_program_label( ...
        ...
        'Pattern', programs(programIdx).pattern, ...
        'Side', oneRow.Side(1), ...
        'Target', oneRow.Target(1), ...
        'RawContact', oneRow.RawContact(1), ...
        'RowIndex', i);
end
end
...
trendTable = mh_coverage_threshold_trend_table(totalRows, {});
fig = mh_viz_trend_line(trendTable.threshold_v_per_mm, trendTable.mean_total_vta_volume_mm3, ...
    'Group', "cohort", ...
...
function validate_cohort_outputs(coverageTable, contactTable, outputDir, thresholdsVPerMm, regionSpec)
...
mh_coverage_validate_threshold_series(totalRows, ...
    {'subject_id', 'phase', 'protocol', 'side'}, thresholdsVPerMm, ...
    'MissingErrorId', 'mh_fiber_run_stnsnr_vta_coverage:MissingThresholdRows', ...
    'MissingMessage', 'Missing threshold rows for %s %s %s %s.', ...
    'MonotonicErrorId', 'mh_fiber_run_stnsnr_vta_coverage:ThresholdMonotonicityFailed', ...
    'MonotonicMessage', 'VTA volume is not monotonic for %s %s %s %s.');

expectedCategoryRows = mh_coverage_region_category_count(regionSpec);
mh_coverage_validate_category_totals(coverageTable, ...
    {'subject_id', 'phase', 'protocol', 'condition_key', 'side', 'threshold_v_per_mm'}, ...
    expectedCategoryRows, ...
    'CountErrorId', 'mh_fiber_run_stnsnr_vta_coverage:MissingCategoryRows', ...
    'CountMessage', sprintf('Expected %d category rows for %%s %%s %%s %%s %%s at %%.2f.', ...
    expectedCategoryRows), ...
    'SumErrorId', 'mh_fiber_run_stnsnr_vta_coverage:CategoryVoxelMismatch', ...
