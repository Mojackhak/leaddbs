...
addParameter(parser, 'Space', mh_vta_default_config_space(), ...
    @(x) ischar(x) || isstring(x));
...
for i = 1:size(data, 1)
    ...
    source.contacts = [ ...
        ...
        struct('contact', char(string(anode)), 'polarity', 'anode', 'fraction', 1.0)];
...
source = struct( ...
    ...
    'pulseWidth', NaN, ...
    'frequency', NaN, ...
    'contacts', repmat(struct('contact', NaN, 'polarity', '', 'fraction', NaN), 0, 1));
end
