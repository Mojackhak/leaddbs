...
class NormativeFiberFormalBackend:
    ...
    def _prepare_permutation_operator_scratch(
        ...
        if request.resampling_kind != "permutation":
            raise FormalBackendInputError(
                "fiber operator scratch requires permutation resampling"
            )
        if not request.final_model.endpoint.model_family.endswith("fiber"):
            raise FormalBackendInputError(
                "fiber operator scratch requires a normative-fiber final"
            )
        if request.connectome_role != "formal":
            raise FormalBackendInputError(
                "fiber operator scratch requires connectome_role='formal'"
            )
        ...
        operators = _build_fold_operators(
            request,
            exposure,
            nuisance_plan,
            _fold_candidate_masks(request, exposure),
        )
