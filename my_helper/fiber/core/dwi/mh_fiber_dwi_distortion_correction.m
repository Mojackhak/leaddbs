...
p.addParameter('B0ReferenceStrategy', 'mean', @(x) ischar(x) || isstring(x));
p.addParameter('B0Threshold', 10, @(x) isnumeric(x) && isscalar(x) && x > 0);
...
if ~ismember(opts.B0ReferenceStrategy, {'mean', 'last'})
    error('mh_fiber_dwi_distortion_correction:InvalidB0ReferenceStrategy', ...
...
referenceWorkDir = fullfile(workDir, 'b0_reference');
reference = mh_fiber_prepare_eddy_b0_reference(paths.dwi, paths.bval, ...
    paths.bvec, referenceWorkDir, opts.B0ReferenceStrategy, ...
    opts.B0Threshold, opts.Force);
distortedB0 = reference.referenceB0;
...
eddyResult = ea_eddy(reference.inputDwi, maskPath, reference.inputBval, ...
    reference.inputBvec, synb0Result.topupPrefix, correctedPrefix, ...
    'PhaseEncodingVector', opts.PhaseEncodingVector, ...
    'TotalReadoutTime', totalReadoutTime, ...
    'Force', opts.Force, ...
    'UseDataIsShelled', true);

if strcmp(opts.B0ReferenceStrategy, 'last')
    eddyUncompressed = fullfile(workDir, [rawBase, '_desc-eddy_reference-first.nii']);
    restoredDwi = fullfile(workDir, [rawBase, '_desc-eddy_source-order.nii']);
    restoredBval = fullfile(workDir, [rawBase, '_desc-eddy_source-order.bval']);
    restoredBvec = fullfile(workDir, [rawBase, '_desc-eddy_source-order.bvec']);
    copy_or_gunzip(eddyResult.correctedDwi, eddyUncompressed, opts.Force);
    mh_fiber_reorder_dwi_series(eddyUncompressed, reference.inputBval, ...
        eddyResult.rotatedBvec, restoredDwi, restoredBval, restoredBvec, ...
        reference.source_to_eddy_one_based, opts.Force);
    copy_or_gunzip(restoredDwi, formalDwi, opts.Force);
    copyfile(restoredBval, formalBval, 'f');
    copyfile(restoredBvec, formalBvec, 'f');
else
...
mh_fiber_extract_b0_reference(formalDwi, formalB0, bvals, ...
...
result.rotatedBvec = formalBvec;
result.eddyRotatedBvec = eddyResult.rotatedBvec;
...
result.b0ReferenceStrategy = opts.B0ReferenceStrategy;
result.b0Threshold = opts.B0Threshold;
result.b0ReferenceSourceIndex = reference.selected_source_index_one_based;
result.b0ReferenceHash = reference.reference_b0_sha256;
result.eddyVolumeMapping = reference.mappingPath;
