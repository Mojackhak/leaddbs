function prepared = mh_viz_prepare_scene_example_input(publicationRoot, modelFamily, varargin)
% Prepare one published final-model display input through Conda leaddbs.

parser = inputParser;
parser.FunctionName = mfilename;
addRequired(parser, 'publicationRoot', @(value) ischar(value) || ...
...
addParameter(parser, 'OutputRoot', fullfile(tempdir, ...
    ...
    (isstring(value) && isscalar(value)));
addParameter(parser, 'Force', false, @(value) islogical(value) && isscalar(value));
parse(parser, publicationRoot, modelFamily, varargin{:});
...
if ~isfolder(publicationRoot)
    error('mh_viz_prepare_scene_example_input:MissingPublication', ...
        'Publication directory does not exist: %s', publicationRoot);
end
...
commandParts = { ...
    ...
    '--publication-root', mh_fiber_shell_quote(publicationRoot), ...
