...
% Generate a native-space 3D scene for SIFT2 seed-VTA-target outputs.
...
try
    ...
    resultfig = figure('Color', 'k', 'Visible', visible_state(openAfterRun), 'Name', 'Seed-VTA SIFT2 scene');
...
handles = struct();
handles.rois = plot_anatomical_seed_rois(resultfig, seedRois, cfg);
handles.vtas = plot_stimulation_vtas(resultfig, vta, cfg);
handles.seedVta = plot_seed_vta_rois(resultfig, result, cfg);
handles.targets = plot_target_rois(resultfig, seedRois, result);
handles.fibers = plot_display_fibers(resultfig, result, cfg);
...
savefig(resultfig, figures.nativeFig);
restore_transient_control_windows(resultfig, transientControls);
export_scene_png(resultfig, figures.nativePng, openAfterRun);
...
function roiHandles = plot_anatomical_seed_rois(resultfig, seedRois, cfg)
roiHandles = gobjects(0);
for sideCell = {'R', 'L'}
    side = sideCell{1};
    roiHandles(end+1) = add_roi(resultfig, seedRois.anchorNative.(side).NAc, ...
        sprintf('NAc %s', side), cfg.figure.colors.NAc, 0.12, 'roi'); %#ok<AGROW>
    roiHandles(end+1) = add_roi(resultfig, seedRois.anchorNative.(side).ALIC, ...
        sprintf('ALIC %s', side), cfg.figure.colors.ALIC, 0.12, 'roi'); %#ok<AGROW>
end
roiHandles = mh_fiber_valid_graphics(roiHandles);
end

function vtaHandles = plot_stimulation_vtas(resultfig, vta, cfg)
vtaHandles = gobjects(0);
for sideCell = {'R', 'L'}
    side = sideCell{1};
    data = load(vta.native.(side).binaryMat, 'vatfv');
    h = patch('Faces', data.vatfv.faces, 'Vertices', data.vatfv.vertices, ...
        ...
        'Tag', ['StimVTA', side], ...
        'UserData', struct('mh_fiber_type', 'stimulation_vta', 'side', side));
...
function seedVtaHandles = plot_seed_vta_rois(resultfig, result, cfg)
...
for sideCell = {'R', 'L'}
    ...
    for seedCell = {'NAc', 'ALIC'}
        seedName = seedCell{1};
        path = fullfile(result.dirs.work, sprintf('%s_hemi-%s_seed-%s_intersect-stimulationVTA_space-anchorNative.nii', ...
            cfg.patientName, side, seedName));
        if ~isfile(path)
            continue;
        end
        key = [seedName, side];
        h = add_roi(resultfig, path, sprintf('%s%sVTA %s', seedName, char(8745), side), ...
            colors.(key), 0.62, 'seed_vta');
...
function targetHandles = plot_target_rois(resultfig, seedRois, result)
...
for sideCell = {'R', 'L'}
    ...
    for i = 1:numel(targets)
        ...
        if ~isfield(seedRois.anchorNative, side) || ~isfield(seedRois.anchorNative.(side), targetName)
            continue;
        end
        color = target_color(targetName);
        label = target_label(targetName, side);
        h = add_roi(resultfig, seedRois.anchorNative.(side).(targetName), label, color, 0.10, 'target_roi');
...
function fiberHandles = plot_display_fibers(resultfig, result, ~)
fiberHandles = gobjects(0);
display = result.displayTable;
for i = 1:height(display)
    path = char(display.native_display_mat(i));
    if ~isfile(path)
        continue;
    end
    ...
    targetName = char(display.target(i));
    side = char(display.side(i));
    seedName = char(display.seed(i));
    color = target_color(targetName);
    h = ea_showfiber(data.fibers(:, 1:3), data.idx, color, 0.36);
    label = sprintf('%s %s%sVTA to %s (%d)', side, seedName, char(8745), targetName, display.display_streamline_count(i));
...
function h = add_roi(resultfig, path, label, color, alphaValue, userData)
h = [];
if ~isfile(path)
    return;
end
...
mh_fiber_add_toggle(resultfig, h, label, color, 'on', userData);
end
...
roiGroup = mh_fiber_valid_graphics([handles.rois(:); handles.seedVta(:); handles.targets(:)]);
