...
Use OSS-DBS / pathway activation modeling as an advanced sensitivity analysis for normative fiber models, not for the direct voxel models. Required OSS sidecars are generated only for final dTOR normative fiber branches unless a future model document explicitly promotes cross-connectome OSS sensitivity. The HF normative fiber OSS branch uses the locked pPAM parameter set defined in `normative_fiber_oss_ppam_generation_plan.md`, inherits the peak-E-field final selected-source candidate fiber id order, replaces the exposure variable with continuous pPAM activation probability, and runs smoke permutation only unless the model document is explicitly revised. The ULF normative fiber OSS branch inherits the realized primary ULF selected-source candidate universe, keeps the matched HF-overlap exclusion rule, replaces `X_ULF_only` with continuous pPAM ULF activation probability when the ULF component is separable, and does not participate in the ULF tau/Coverage source resolver or endpoint-status assignment.
...
- Candidate / coverage: use the MNI152NLin2009bAsym `brainmask.nii.gz > 0` right hemisphere (`x > 0`) as the canonical reference mask. Build a sparse candidate mask from any valid subject with `X_HF_only > 180 V/m` for the pre-specified `tau200/Coverage>=5` source; the Round 2 all-endpoint tau/Coverage resolver uses its declared scan grid and the dedicated low-threshold sidecar when lower tau cells are evaluated. Do not intersect `Omega_HF_tau` with `right_STNSNrplus`; keep `STNSNr-connected regions/{rh,lh}/STNSNrplus.nii.gz` only as anatomical overlay and coverage/QC background. Selected-source tau sensitivity reports `0.9 * selected_tau` and `1.1 * selected_tau` at the selected Coverage, so a scan-fallback source is tested around its own tau rather than around 200 V/m. Coverage-neighborhood summaries are secondary support diagnostics.
...
- Permutation: patient-level Freedman-Lane, formal `B = 10000`, smoke/exploratory `B = 1000`, seed `42`, primary statistic LOOCV Spearman rho, plus-one two-sided p value. Formal permutation is restricted to the endpoint's final source branch; non-selected neighborhood branches record `not_run_nonfinal`. The primary `coef` map stores `rho_HF(v)`; the OLS supplemental `coef` map stores `theta_HF(v)`. No per-voxel FDR is applied.
- Bootstrap: subject-level full-process bootstrap, formal `B = 10000`, smoke/exploratory `B = 1000`, seed `42`; rerun map building including `Omega_HF_tau` and store voxel-wise estimator standard error for the final source branch only. Non-selected neighborhood branches do not generate bootstrap SE maps.
- Spatial jitter QC: final source branch only, formal `B = 1000`, smoke `B = 100`, independent subject-side 3D translation jitter with Gaussian `FWHM = 2 mm`, linear E-field interpolation, and outside fill `0`.
- Endpoints: all available HF-only endpoints in `subject_effect_origin.xlsx` are processed through the same resolver when requested; `MDS-UPDRS III axial score` has no special engineering status. A patient missing `Y_HF3m` or a side's e-field fails scale-level QC as specified in the model summary.
- Outputs: keep the `direct_voxel_HF_*` file names under `/Volumes/VAL/STNSNr/summary/direct_voxel/hf/<scale_slug>/<tau_or_selected_source>/partial_spearman/`; optional future OLS outputs would use sibling `ols_ancova/` directories if explicitly enabled. Unsmoothed maps are primary, `1-2 mm` FWHM smoothed maps are display-only outputs. Report-only sweet/sour display masks use top 10% same-sign map values plus direction-specific stability `>=0.75`; they are not significance maps.
...
meaning at least 50% of patients have suprathreshold exposure at that voxel. This generic reference rule is not used to define the primary HF or ULF direct voxel results. The executable HF and ULF direct voxel model summaries use model-specific resolver rules; for HF direct voxel, higher coverage cells are evaluated only inside the Round 2 tau/Coverage resolver or selected-source neighborhood report.
...
- For HF and ULF normative fiber-level models, output fiber-wise FDR q-values and anatomical/pathway enrichment caches for QC/display/interpretation only. For individualized-DWI target-level models, correct multiple comparisons across tested targets within each scale, DWI source, endpoint, and model class using FDR.
- Use patient-level permutation tests with seed `42` for empirical significance.
- Normative fiber FDR and anatomical/pathway enrichment cache definitions are maintained in `my_helper/stnsnr/normative_fiber_fdr_enrichment_cache_definition.md`; these caches are QC/display/interpretation layers and stay outside resolver and final-branch selection.
...
12. Fit endpoint-specific ULF chronic add-on gain direct voxel and normative fiber models; direct voxel runs both no-DeltaHF and DeltaHF-adjusted branches when a stable HF voxel source exists, and normative fiber follows its model-specific resolver.
13. Fit endpoint-specific ULF immediate add-on gain direct voxel and normative fiber models with same-day `Y_HF_ref`; include `DeltaHFScore_immediate` when the matched HF source exists and the model-specific resolver allows it.
