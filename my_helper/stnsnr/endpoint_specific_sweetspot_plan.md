...
- Estimator: primary baseline-adjusted partial Spearman with average ranks and benefit-oriented `rho`; OLS ANCOVA is a full parallel supplemental estimator. The primary patient-level score is the unnormalized `HFScore_sum = sum_v X_HF_only(v) * M_HF(v)`, without division by `sum(X)` and without voxel-volume multiplication. The normalized score is descriptive only.
