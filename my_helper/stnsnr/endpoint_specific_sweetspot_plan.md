...
- Permutation: patient-level Freedman-Lane, formal `B = 10000`, smoke/exploratory `B = 1000`, seed `42`, primary statistic LOOCV Spearman rho, plus-one two-sided p value. Formal permutation is restricted to the endpoint's selected source branch; non-selected neighborhood branches record `not_run_nonprimary`. The primary `coef` map stores `rho_HF(v)`; the OLS supplemental `coef` map stores `theta_HF(v)`. No per-voxel FDR is applied.
- Bootstrap: subject-level full-process bootstrap, formal `B = 10000`, smoke/exploratory `B = 1000`, seed `42`; rerun map building including `Omega_HF_tau` and store voxel-wise estimator standard error for the selected source branch only. Non-selected neighborhood branches do not generate bootstrap SE maps.
- Spatial jitter QC: selected source branch only, formal `B = 1000`, smoke `B = 100`, independent subject-side 3D translation jitter with Gaussian `FWHM = 2 mm`, linear E-field interpolation, and outside fill `0`.
