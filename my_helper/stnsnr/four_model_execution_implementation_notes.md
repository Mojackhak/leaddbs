...
`code_provenance` injection for manifest-like JSON outputs. The consolidated
execution status layer, formal target worklist, formal readiness audit, final
reporting layer, all-endpoint reporting layer, C same-day immediate driver, and
C/D gain/total-ULF sensitivity drivers use this helper. Migration should remain
incremental and behavior-preserving. It must not change model formulas, source
resolver rules, branch-role assignment, or output paths.
