...
The former model-set-wide full-payload validator is retired. Its model-set
identity field, versioned wrapper paths, whole-publication hash closure, and
same-byte report collision policy are not part of the current runtime or
publication contract. Normal execution validates the parsed YAML at its input
boundary and resumes individual tasks from their deterministic paths and
`complete.json` markers. Explicit output validation remains responsible for
checking the publication contract without becoming a resume gate.
