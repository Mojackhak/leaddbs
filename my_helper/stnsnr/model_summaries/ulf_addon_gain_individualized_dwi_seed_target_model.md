# HF-Adjusted ULF-Only Add-On Gain Individualized DWI Seed-Target Model

## Research Question

Which subject-specific ULF-only DWI seed-target connectivity features are associated with additional benefit after ULF stimulation is added to HF stimulation, after adjusting for HF clinical state and predicted change in HF target engagement?
...
```text
DeltaHFScore =
  S_HF_ind(C_ind_HF_component,HF+ULF)
...
For each subject-specific streamline `l`, define HF and ULF activation:
...
C_ind_LF_only_side_i(k) =
  aggregate_l w_ULF_only_i(l) for subject-specific streamlines l assigned to target k
```
...
Y_HFplusULF_post_i = alpha_k
                  + theta_ULF(k) * C_ind_LF_only_bilat_i(k)
                  + beta_k      * Y_HF3m_i
...
Patient-level individualized ULF-only target score:

```text
ULFTargetScore_ind_i =
  sum_k C_ind_LF_only_bilat_i(k) * W_ULF(k)
...
- Select ULF targets, fit ULF weights, and compute `ULFTargetScore_ind` inside training folds.
...
```text
individualized_ULF_only_target_weights.csv
individualized_ULF_only_target_scores.csv
individualized_ULF_only_loocv_predictions.csv
individualized_ULF_only_permutation_summary.csv
individualized_ULF_only_streamline_contribution_map.nii.gz
individualized_ULF_only_subject_qc.csv
individualized_ULF_only_HF_overlap_exclusion_summary.csv
```

Visualize subject-specific ULF-only target weights, ULF-only streamline contribution density, and excluded HF-overlap streamlines with STN/SNr territory overlays.

## Interpretation Boundary

This model estimates HF-adjusted ULF-only target-level association using individualized DWI. It is more anatomically personalized than the normative model, but it remains limited by DWI quality, tractography uncertainty, registration, small sample size, and the rule that HF/ULF-overlap streamlines are assigned to HF rather than ULF.
