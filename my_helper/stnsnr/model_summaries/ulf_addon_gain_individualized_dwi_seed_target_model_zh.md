# HF-adjusted ULF-only Add-On Gain Individualized DWI Seed-Target 模型

## 研究问题

加入 ULF 刺激后，哪些 subject-specific ULF-only DWI seed-target connectivity 特征与额外疗效相关，并且这种关联已校正 HF 临床状态和预测的 HF target engagement 变化？
...
对每条 subject-specific streamline `l`，定义 HF 和 ULF activation：
...
患者层面的 individualized ULF-only target score：
...
- ULF target 选择、ULF weight 拟合和 `ULFTargetScore_ind` 计算均在 training fold 内完成。
...
可视化 subject-specific ULF-only target weight、ULF-only streamline contribution density 和被排除的 HF-overlap streamline，并叠加 STN/SNr territory overlay。

## 解释边界

该模型使用 individualized DWI 估计 HF-adjusted ULF-only target-level association。它比 normative model 更个体化，但仍受 DWI 质量、tractography uncertainty、registration、小样本以及“HF/ULF overlap streamline 归入 HF 而非 ULF”这一规则限制。
