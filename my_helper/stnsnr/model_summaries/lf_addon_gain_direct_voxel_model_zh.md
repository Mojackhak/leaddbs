# HF-adjusted LF-only Add-On Gain 直接 Voxel-Level 模型

## 研究问题

在加入 LF 刺激后，哪些 LF-only voxel 与额外疗效相关，并且这种关联在模型中已经校正患者的 HF 临床状态和预测的 HF 疗效变化？

这是一个局部 LF-only sweet spot 模型。主问题不再定义为解剖 SNr 效应。STN/SNr 及周围区域被视为一个 stimulation territory，HF 与 LF component 按刺激频率和 exposure overlap 进行区分。
...
- pre-LF 阶段的 HF-only stimulation field。
- HF+LF programming 中的 HF component field 和 LF component field。
...
LF predictor 只使用 LF-only exposure：
...
如果某个 voxel 同时被 HF 和 LF component 激活，它归入 HF model，并通过 `DeltaHFScore` adjustment 进入模型，而不是进入 `X_LF_only`。

左右侧 LF-only feature 分别计算后求平均：
...
使用 `0.18` 和 `0.22 V/mm` 做阈值敏感性分析。还需报告 total-field 和 shape-normalized LF sensitivity，但主预测变量保持为 LF-only exposure。
...
- 与 covariate-only model `Y_HFplusLF_post ~ Y_HF3m + DeltaHFScore` 比较。
...
显示 LF-only sweet/sour map，同时叠加被排除的 HF-overlap 区域以及 STN/SNr 解剖边界。

## 解释边界

该模型估计 HF-adjusted LF-only add-on association。它不是解剖 SNr gain 模型。被 HF 与 LF 同时激活的 voxel 在主分析中视为 HF-dominant，因此 LF map 表示在校正 HF 疗效变化后由 LF 唯一招募的区域。
