...
| Independent OSS axis equivalence and pPAM | ACTIVE | `task17-oss-v1-inclusive-formal-20260719` now has a stable terminal reference group of 34 decisions and 68 rows, all pass, produced by Decision 39 cache-only promotion without a reference solver call. The add-on gate retains three validated pass decisions and six complete rows and reaches the solver only for missing work. Guarded `segment_0017` ended with 393 completed tasks, one failed add-on gate, and 196 dependency skips after a later MATLAB preprocessing launch returned code 1 with empty logs. RSS peaked at 20272988160 bytes, swap growth was `< 1` byte, and VAL remained mounted. A bounded post-failure MATLAB batch returned success in about 14 seconds, supporting a transient external startup classification. Existing scientific payloads remain valid and preserved. | Resume the same lineage as a new guarded segment. Require stable reference restoration, reuse of all completed add-on cache, terminal add-on closure, execution of all dependency-derived downstream tasks, zero terminal failures, and complete reporting, artifact-index, and resource validation. |
...
- `run_task17_performance_matrix.py` for its current `prepare` and read-only
  `validate` operations;
...
matrix. The repository-owned harness now prepares and revalidates the immutable
request, maximum-burden workload selection, DAG slices, and 72-row closure, but
does not yet expose its measured-row `run` operation. No production matrix
manifest exists yet. Both remaining execution implementation and production
execution therefore remain open. The dedicated
