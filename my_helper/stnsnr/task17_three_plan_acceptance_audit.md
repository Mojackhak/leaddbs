...
| Resource scheduling and guard contract | ACTIVE | One persistent spawn pool, 14-worker ceiling, single solver token, 48-GiB admission charge, task-tree RSS `< 64 GiB`, and swap growth `< 1` byte are implemented and covered by static tests. The strict validator now closes safe segment identity, managed-memory and reserve declarations, connectome-I/O/solver/BLAS ceilings, CPU/memory/I/O/solver/running-task peaks, exact admission maps, contained SHA-bound scheduler windows, and a final zero-RSS guard terminal row. The checked-in guard plus pre-instrumentation and strict validators pass 40 focused tests. Segment 0015 remains on its separate pre-instrumentation path and is guarded by the checked-in one-second observer. | Build the repository-owned maximum-row measurement window from the terminal segment and pass the independent pre-instrumentation validator. Repeat strict acceptance for combined. |
