...
| Independent OSS axis equivalence and pPAM | ACTIVE | `task17-oss-v1-inclusive-formal-20260719` retains a terminal reference closure of 34 decisions and 68 rows, all pass, plus three validated add-on decisions and six complete rows. Decision 39 compatibility was merged only after the interrupted writer exited; lineage IDs, parent manifest SHA, scientific configuration, and OSS-only scope validated. Guarded `segment_0016` restored 196 tasks, scheduled zero tasks, invoked no solver, and wrote no cache entry. It exposed a scheduler-only defect: the forced cache-only reference replay still received a 48-GiB solver grant, while both ready gates were temporarily above the 49758738842-byte dynamic managed-memory ceiling and the empty running set was misclassified as deadlock. Decision 40 now separates cache-read admission from solver admission and requires dynamic memory pressure to remain pending. Its focused coverage passes 157 tests plus 36 subtests; complete discovery passes 727 tests plus 326 subtests with warnings treated as errors. Existing reference and add-on scientific payloads remain valid. | Resume as a new guarded segment. Require cache-only reference promotion, reuse of all three completed add-on decisions, solver work only for missing add-on rows, terminal two-gate closure, execution of all 392 dependency-derived downstream tasks, zero terminal failures, and complete reporting, artifact-index, and resource validation. |
