...
if ~ismember(mode, {'fem', 'fixture_only', 'compare_existing'})
    error('run_single_current_backend_equivalence:InvalidMode', ...
        'Mode must be fem, fixture_only, or compare_existing.');
end
...
if strcmp(mode, 'compare_existing')
    ...
    result = compare_existing_run(runRoot, opts.TrashRoot);
    return;
end
...
if nnz(isSelected) ~= 1
    error('run_single_current_backend_equivalence:OfficialFemCaseMissing', ...
...
plan = struct( ...
    ...
    'backends', {backends}, ...
    'execution_paths', {{'standard_simbio', 'canonical_onesolve'}}, ...
    'runs_per_backend', 1, ...
...
for caseIndex = 1:numel(cases)
    ...
    for backendIndex = 1:numel(femPlan.backends)
        ...
        switch executionPath
            ...
            case 'canonical_onesolve'
                standardOutput = outputs.(caseField).simbio;
                outputs.(caseField).(backendField) = ...
                    execute_canonical_onesolve(copiedSubject, subject, ...
                        oneCase, atlasSet, runRoot, standardOutput.native);
            otherwise
                error('run_single_current_backend_equivalence:InvalidExecutionPath', ...
...
function output = execute_canonical_onesolve(copiedSubject, subject, ...
...
nativeLeaf = fullfile(runRoot, 'outputs', 'simbio_onesolve', ...
    oneCase.case_id, 'native');
mniLeaf = fullfile(runRoot, 'outputs', 'simbio_onesolve', ...
...
if ~strcmp(status.headmodel_state, 'reused')
    error('run_single_current_backend_equivalence:HeadmodelNotReused', ...
...
function result = compare_existing_run(runRoot, trashRoot)
...
if ~all(isfield(manifest, required)) || ...
    ...
    error('run_single_current_backend_equivalence:InvalidExistingRun', ...
...
candidateRoot = fullfile(runRoot, 'outputs', 'simbio_onesolve', caseId);
...
outputs.(caseField).simbio_onesolve = struct( ...
    'native', first_existing_file({ ...
...
if nnz(matching) ~= 1
    error('run_single_current_backend_equivalence:ElectrodeSelectionFailed', ...
