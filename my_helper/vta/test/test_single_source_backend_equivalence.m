...
optimizationCandidatePath = fullfile( ...
    testRoot, 'optimization_candidate.nii');
optimizationCandidate = referenceData;
optimizationCandidate(100) = optimizationCandidate(100) + 0.002;
write_nii_like(referencePath, optimizationCandidatePath, optimizationCandidate);
identicalOptimizationResult = mh_compare_single_source_backend_outputs( ...
    referencePath, referencePath, ...
    'Comparison', 'optimization_regression');
assert(identicalOptimizationResult.pass, ...
    'Identical inputs must pass optimization regression.');

optimizationDoubleReferencePath = fullfile( ...
    testRoot, 'optimization_double_reference.nii');
optimizationBelowPath = fullfile( ...
    testRoot, 'optimization_below.nii');
optimizationBoundaryPath = fullfile( ...
    testRoot, 'optimization_boundary.nii');
optimizationDoubleReference = referenceData;
optimizationBelow = optimizationDoubleReference;
optimizationBelow(100) = optimizationBelow(100) + 0.0005;
optimizationBoundary = optimizationDoubleReference;
optimizationBoundary(100) = optimizationBoundary(100) + 0.001;
write_double_nii(optimizationDoubleReferencePath, ...
    optimizationDoubleReference);
write_double_nii(optimizationBelowPath, optimizationBelow);
write_double_nii(optimizationBoundaryPath, optimizationBoundary);
belowResult = mh_compare_single_source_backend_outputs( ...
    optimizationDoubleReferencePath, optimizationBelowPath, ...
    'Comparison', 'optimization_regression');
boundaryResult = mh_compare_single_source_backend_outputs( ...
    optimizationDoubleReferencePath, optimizationBoundaryPath, ...
    'Comparison', 'optimization_regression');
assert(belowResult.pass && belowResult.efield_row.value_max_abs < 1e-3, ...
    'A sub-threshold difference must pass optimization regression.');
assert(boundaryResult.pass && ...
    abs(boundaryResult.efield_row.value_max_abs - 1e-3) <= 1e-12, ...
    'The documented inclusive 1e-3 V/m boundary must pass.');
assert_error_id(@() mh_compare_single_source_backend_outputs( ...
    referencePath, referencePath, ...
    'Comparison', 'optimization_regression', ...
    'ResampleCandidateToReference', true), ...
    'mh_compare_single_source_backend_outputs:InvalidComparison');

standardResult = mh_compare_single_source_backend_outputs( ...
    referencePath, optimizationCandidatePath, ...
    'Comparison', 'equivalence', 'ThrowOnFailure', false);
optimizationResult = mh_compare_single_source_backend_outputs( ...
    referencePath, optimizationCandidatePath, ...
    'Comparison', 'optimization_regression', 'ThrowOnFailure', false);
assert(standardResult.pass, ...
    'A difference below 0.05 V/m must pass standard equivalence.');
assert(~optimizationResult.pass && ...
    optimizationResult.efield_row.value_max_abs > 1e-3 && ...
    optimizationResult.efield_row.value_max_abs < 0.05, ...
    'Optimization regression must enforce its independent 1e-3 V/m gate.');
assert_error_id(@() mh_compare_single_source_backend_outputs( ...
    referencePath, optimizationCandidatePath, ...
    'Comparison', 'optimization_regression'), ...
    'mh_compare_single_source_backend_outputs:ValueToleranceExceeded');
...
function write_double_nii(path, data)
niftiwrite(double(data), path, 'Compressed', false);
end
