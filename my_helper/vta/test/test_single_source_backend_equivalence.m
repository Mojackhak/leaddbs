...
assert(result.efield_row.value_max_abs <= 0.05, ...
...
valueData(100) = valueData(100) + 0.051;
...
crossingReferencePath = fullfile(testRoot, 'crossing_reference.nii');
crossingCandidatePath = fullfile(testRoot, 'crossing_candidate.nii');
crossingReference = referenceData;
crossingCandidate = referenceData;
crossingReference(100) = 199.98;
crossingCandidate(100) = 200.02;
write_nii_like(referencePath, crossingReferencePath, crossingReference);
write_nii_like(referencePath, crossingCandidatePath, crossingCandidate);
crossingResult = compare_equivalence( ...
    crossingReferencePath, crossingCandidatePath);
assert(crossingResult.pass, ...
    ['A near-threshold discordance must pass when native E-field, Dice, ', ...
     'and volume gates pass.']);
