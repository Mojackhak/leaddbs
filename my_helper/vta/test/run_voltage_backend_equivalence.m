function result = run_voltage_backend_equivalence(varargin)
% Run a copied-subject numerical equivalence gate for two SimBio backends.

parser = inputParser;
parser.FunctionName = 'run_voltage_backend_equivalence';
...
for caseIndex = 1:numel(cases)
    ...
    for backendIndex = 1:numel(backends)
        ...
        for repeat = 1:2
            nativeAnchor = '';
            if strcmp(backend, 'canonical')
                nativeAnchor = outputIndex.(caseField).simbio(1).native;
            end
            [paths, beforeHash, afterHash, effectiveDefaults, ...
                diagnostics, label] = run_one_backend(copiedSubject, ...
                subject, oneCase, backend, repeat, atlasSet, runRoot, seed, ...
                nativeAnchor);
...
mniContract = validate_canonical_mni_outputs(cases, outputIndex);
...
if ~isstruct(document) || ~isfield(document, 'schema_version') || ...
    ...
    error('run_voltage_backend_equivalence:InvalidStudyBase', ...
...
if nnz(subjectMask) ~= 1
    error('run_voltage_backend_equivalence:SubjectSelectionFailed', ...
...
if nnz(phaseMask) ~= 1
    error('run_voltage_backend_equivalence:PhaseSelectionFailed', ...
...
if nnz(programMask) ~= 1
    error('run_voltage_backend_equivalence:ProgramSelectionFailed', ...
...
for epIndex = 1:numel(program.electrode_programs)
    ...
    if numel(groups) ~= 1 || ~strcmp(char(string(groups.delivery_mode)), 'continuous') || ...
        ...
        error('run_voltage_backend_equivalence:NotSingleContinuousSource', ...
    ...
    if localContact < 1 || localContact > double(electrode.contact_count)
        error('run_voltage_backend_equivalence:ContactOutOfRange', ...
    ...
    oneCase.reconstruction_lead_id = ...
        double(electrode.reconstruction_lead_id);
...
if numel(cases) ~= 2 || ~isequal(sort(string({cases.side})), ["L" "R"])
    error('run_voltage_backend_equivalence:BilateralCasesRequired', ...
...
if nnz(mask) ~= 1
    error('run_voltage_backend_equivalence:ElectrodeSelectionFailed', ...
...
if ~strcmp(char(string(source.control_mode)), 'voltage')
    error('run_voltage_backend_equivalence:UnsupportedControlMode', ...
...
for i = 1:numel(requiredPositiveFields)
    ...
    if ~isfield(source, field) || ~isnumeric(source.(field)) || ...
            ...
            source.(field) <= 0
        error('run_voltage_backend_equivalence:InvalidSourceParameter', ...
...
if nnz(cathodes) ~= 1 || nnz(caseAnodes) ~= 1 || numel(contacts) ~= 2
    error('run_voltage_backend_equivalence:UnsupportedContactDesign', ...
...
prefix = 'vta_voltage_backend_equivalence_';
[~, leaf] = fileparts(workRoot);
isExactRoot = startsWith(leaf, prefix);
if reuse
    if ~isExactRoot || ~isfolder(workRoot)
        error('run_voltage_backend_equivalence:InvalidPreparedRoot', ...
    ...
    if isfolder(fullfile(workRoot, 'outputs')) || ...
        ...
        error('run_voltage_backend_equivalence:PreparedRootNotOutputFree', ...
...
if isExactRoot && isfolder(workRoot)
    error('run_voltage_backend_equivalence:ExistingValidationRoot', ...
...
if ~ok
    error('run_voltage_backend_equivalence:SubjectCopyFailed', ...
...
if numel(markerIndex) ~= 1
    error('run_voltage_backend_equivalence:InvalidBidsSubjectPath', ...
...
if ~ok
    error('run_voltage_backend_equivalence:MetadataCopyFailed', ...
...
if isfolder(destination) || isfile(destination)
    error('run_voltage_backend_equivalence:ExistingHeadmodelArchive', ...
        'Head-model archive already exists: %s', destination);
end
[ok, message] = movefile(source, destination);
if ~ok
    error('run_voltage_backend_equivalence:HeadmodelArchiveFailed', ...
...
if ~isempty(beforeHash)
    error('run_voltage_backend_equivalence:HeadmodelPreparationNotFresh', ...
...
function [paths, beforeHash, afterHash, effectiveDefaults, diagnostics, label] = ...
        run_one_backend(copiedSubject, subject, oneCase, backend, repeat, ...
        atlasSet, runRoot, seed, nativeAnchor)
...
if strcmp(backend, 'canonical')
    paths = execute_canonical_backend(copiedSubject, subject, oneCase, ...
        atlasSet, targetDir, nativeAnchor);
    afterHash = mh_fiber_file_sha256(must_be_file(headmodel, ...
        'shared head model'));
    diagnostics = struct('seed_used', seed, 'attempt_count', 1, ...
        'attempt_seeds', seed);
    effectiveDefaults = extract_effective_defaults(struct());
else
    [taskResults, ~, afterHash, effectiveDefaults, diagnostics] = ...
...
function paths = execute_canonical_backend(copiedSubject, subject, oneCase, ...
        atlasSet, targetDir, nativeAnchor)
...
nativeLeaf = fullfile(targetDir, 'native');
mniLeaf = fullfile(targetDir, 'MNI152NLin2009bAsym');
contacts = [ ...
    struct('contact', oneCase.local_contact, ...
        'polarity', 'cathode', 'fraction', 1.0), ...
    struct('contact', 'case', 'polarity', 'anode', 'fraction', 1.0)];
source = struct( ...
    'source_id', char(string(oneCase.source.source_id)), ...
    'frequency_hz', double(oneCase.source.frequency_hz), ...
    'control_mode', 'voltage', ...
    'amplitude', double(oneCase.source.amplitude), ...
    'pulse_width_us', double(oneCase.source.pulse_width_us), ...
    'contacts', contacts);
allArtifacts = { ...
    'efield.nii.gz', ...
...
task = struct( ...
    'task_id', sprintf('voltage-acceptance-%s-%s', ...
        lower(oneCase.side), char(string(oneCase.source.source_id))), ...
    ...
    'phase_id', 'acceptance', ...
    'program_id', 0, ...
    'electrode_id', oneCase.electrode_id, ...
    'hemisphere', oneCase.side, ...
    'electrode_model', oneCase.electrode_model, ...
    'reconstruction_lead_id', oneCase.reconstruction_lead_id, ...
    'frequency_group_id', 'voltage-acceptance', ...
...
status = mh_vta_backend_simbio_onesolve_canonical(task, ...
    'NativeAnchorPath', nativeAnchor);
if ~strcmp(status.headmodel_state, 'reused')
    error('run_voltage_backend_equivalence:HeadmodelNotReused', ...
        'Canonical backend must reuse the standard SimBio head model.');
end
paths = struct( ...
...
if numel(taskResults) ~= 1 || ~strcmp(taskResults.status, 'complete')
    error('run_voltage_backend_equivalence:BackendRunFailed', ...
...
if ~isfield(taskResults, 'rng_diagnostics')
    error('run_voltage_backend_equivalence:MissingRngDiagnostics', ...
...
if ~ok
    error('run_voltage_backend_equivalence:OutputCopyFailed', ...
...
elseif ~isequaln(resolved, candidate)
    error('run_voltage_backend_equivalence:EffectiveDefaultsChanged', ...
...
if official && ~hardwareMatches
    error('run_voltage_backend_equivalence:OfficialHardwareMismatch', ...
...
for i = 1:numel(cases)
    ...
    canonical = outputs.(field).canonical;
    for s = 1:numel(spaces)
        ...
        repeatCanonical = mh_compare_single_source_backend_outputs( ...
            canonical(1).(space), canonical(2).(space), ...
            'Side', oneCase.side, 'Space', space, ...
            'Comparison', 'repeatability', 'ThrowOnFailure', false);
        equivalence = mh_compare_single_source_backend_outputs( ...
            standard(1).(space), canonical(1).(space), ...
            'Side', oneCase.side, 'Space', space, ...
        ...
        repeatCells{comparisonIndex * 2} = annotate_efield( ...
            repeatCanonical.efield_row, oneCase.case_id, 'canonical');
        ...
        binaryCells{comparisonIndex} = annotate_binary( ...
            ...
            'simbio_vs_canonical');
        passed = passed && repeatStandard.pass && repeatCanonical.pass && ...
            equivalence.pass;
    end
end
...
function result = validate_canonical_mni_outputs(cases, outputs)
rows = table();
passed = true;
...
for index = 1:numel(cases)
    ...
    thresholdPaths = threshold_artifact_paths( ...
        fileparts(canonical(1).mni), thresholds);
    validation = mh_validate_canonical_mni_outputs( ...
        canonical(1).mni, canonical(2).mni, thresholdPaths, thresholds, ...
...
manifest.backends = {'simbio', 'canonical'};
...
relativePaths = { ...
    ...
    ['my_helper/fiber/core/stimulation/model/backends/', ...
        'mh_vta_backend_simbio_onesolve_canonical.m'], ...
...
if numel(matches) ~= 1
    error('run_voltage_backend_equivalence:TransformSelectionFailed', ...
...
fid = fopen(path, 'w');
if fid < 0
    error('run_voltage_backend_equivalence:SummaryWriteFailed', ...
...
fid = fopen(path, 'w');
if fid < 0
    error('run_voltage_backend_equivalence:JsonWriteFailed', ...
...
if ~isfile(path)
    error('run_voltage_backend_equivalence:MissingFile', ...
...
path = char(string(value));
if ~isfolder(path)
    error('run_voltage_backend_equivalence:MissingDirectory', ...
        '%s does not exist: %s', label, path);
end
end
