...
- [ ] Apply the exact unstaged three-file diff from `/Users/mojackhu/Github/leaddbs` to this isolated worktree without modifying the primary worktree.
- [ ] Run `conda run -n leaddbs python -m unittest projects.stnsnr.importer.tests.test_study_base` from `my_helper/fiber` and require all 25 baseline tests to pass.
- [ ] Run `git diff --check`.
- [ ] Commit as `feat: preserve raw stimulation components in study base`.
...
- [ ] Write failing tests for voltage contacts resolving to all `1.0`.
- [ ] Write failing tests for complete observed current fractions `0.7/0.3` and complete absolute currents `7/3`.
- [ ] Write failing tests for all-missing current allocation resolving to `1 / n` separately for cathodes and anodes.
- [ ] Write failing tests proving partial current allocation and mixed fraction/current inputs fail.
- [ ] Run the focused tests and verify failure because the module is absent.
- [ ] Implement the resolver using polarity-index groups and `math.isfinite`; never normalize incomplete values.
- [ ] Run the focused tests and require success.
- [ ] Commit as `feat: add canonical contact fraction resolver`.
...
- [ ] Add failing fixture tests for voltage multi-contact, current explicit allocation, current equal inferred allocation after resolution, bipolar electrode return, and multipolar case return.
- [ ] Add failing tests for alternating one-source groups, duplicate contacts/case, missing polarity, voltage fraction other than `1.0`, current polarity sums other than `1.0`, and mixed source frequencies.
- [ ] Run importer tests and verify failures reflect current `control_mode: const voltage`, one-cathode/case-only semantics, and missing delivery cardinality checks.
- [ ] Change `control_mode` to enum `voltage | current`; keep all field names unchanged.
- [ ] Add Schema `if/then` cardinality: continuous `minItems: 1`, alternating `minItems: 2`.
- [ ] Replace one-cathode/case-only semantic validation with duplicate-contact, polarity-presence, return-mode, fraction, and same-frequency validation.
- [ ] Keep the STNSNr workbook adapter voltage-only and emit its current cathode/case values through the voltage resolver.
- [ ] Run resolver and importer suites and require success.
- [ ] Commit as `feat: validate multi-contact stimulation sources`.
...
- [ ] Write a MATLAB test covering legacy normalization, voltage multi-contact, current equal/explicit fraction validation, bipolar return, duplicate contacts, and partial/invalid sums.
- [ ] Run MATLAB batch and verify the test fails on missing canonical contact support.
- [ ] Normalize legacy sources into contact arrays in `mh_fiber_set_stimulation`.
- [ ] Validate side, unit, source amplitude/frequency/pulse width, unique contacts, required polarities, case-return exclusivity, and control-mode-specific fractions.
- [ ] Update table conversion to produce canonical contact arrays while preserving existing public arguments.
- [ ] Update label generation to preserve legacy labels and deterministically encode multiple numeric contacts.
- [ ] Run the MATLAB test and require success.
- [ ] Commit as `feat: normalize multi-contact stimulation specs`.
...
- [ ] Extend the MATLAB test with a synthetic `S` and expected voltage/current `perc` and `pol` values.
- [ ] Run the test and verify failure because the pure mapping helper is absent.
- [ ] Implement the pure helper and replace single-contact writes in `mh_fiber_build_stimulation`.
- [ ] Verify monopolar case return, bipolar electrode return, and multipolar current allocation.
- [ ] Run the focused MATLAB test and existing static MATLAB tests.
- [ ] Commit as `feat: map contact fractions into Lead-DBS stimulation`.
...
- [ ] Write failing MATLAB tests for continuous one/multi-source expansion, alternating two-source expansion, one-source alternating rejection, frequency mismatch, and unsupported joint-current failure.
- [ ] Run the test and verify expected failure.
- [ ] Implement delivery expansion without duty-cycle or time-weighted fields.
- [ ] Add explicit capability flags: `simbio_onesolve` supports joint voltage only; no current backend claims joint current until a simultaneous current solve exists.
- [ ] Run focused MATLAB tests and require success.
- [ ] Commit as `feat: expand VTA delivery groups deterministically`.
...
- [ ] Validate both against their corresponding Schema.
